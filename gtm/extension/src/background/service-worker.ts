...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === 'install') {
    ...
    chrome.tabs.create({
      url: 'https://github.com/bhaktofmahakal/My-Automations/extension/welcome',
    });
  }
});
...
async function fetchRecommendations(context: Record<string, string>) {
  ...
  const response = await fetch(
    `https://github.com/bhaktofmahakal/My-Automations/api/v1/prompts/recommend?${params.toString()}`
  );
